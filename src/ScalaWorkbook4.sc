...
//method 2//
def tooHot2 (temperature:Int, isSummer:Boolean): Boolean = (temperature, isSummer) match {

  case (temperature, isSummer) if temperature >= 60 && temperature <= 90 && !isSummer => true
  case (temperature, isSummer) if temperature >= 60 && temperature <= 100 && isSummer => true
  case _ => false
